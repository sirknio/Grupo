...
class Evento extends CI_Controller {
	...
	public function index($idGrupo = '', $id = '') {
		...
		$this->session->set_userdata('GrupoInsert', $idGrupo);
	...
	public function insertItem($createId = '') {
		...
		If($createId === '') {
			$this->loadData($data,$this->debug);
			$_POST['idGrupo'] = $data['userdata']['GrupoInsert'];
			// echo"<pre>";echo $_POST['idGrupo'];echo"</pre>";
			$this->loadHTML($data);
			$this->load->view('pages/'.$this->pagecard,$data);
		} else {
			if ($_POST['idGrupo'] == '') unset($_POST['idGrupo']);
			$data['insert'] = $_POST;
